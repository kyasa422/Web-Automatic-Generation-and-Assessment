...
const Esai = () => {
    ...
    const geminiApiKey = "AIzaSyBfeeUiCYcPR_xkHGCPshe2GnN7c_Exd7Y";
    ...
    function cleanJsonString(str) {
        // Ganti kutip miring “ dan ” jadi kutip biasa "
        let cleaned = str.replace(/[“”]/g, '"');

        // Hapus karakter aneh yang biasa muncul dari copy-paste atau AI output, misal karakter non-UTF8
        cleaned = cleaned.replace(/[^\x20-\x7E\n\r\t]+/g, "");

        // Bisa juga tambahkan fix lain kalau tahu pola error JSON yang sering muncul
        ...
        try {
            try {
                ...
                const model = genAI.getGenerativeModel({
                    model: "tunedModels/dataset-fine-tune-e6dw24pc7u1j9p49csht7a",
                    generationConfig: {
                        temperature: 1,
                        topP: 0.95,
                        topK: 40,
...
Semua soal (ESSAY dan MULTIPLE CHOICE) harus dimasukkan ke dalam satu array "response".`;
                ...
                try {
                    ...
                    setQuestion("response", json.response); // sesuaikan dengan data state kamu
                } catch (err) {
    ...
    const handleChangeQuestion = (value, index, type) => {
        ...
        const newTotal = temp.response.reduce(
            (sum, q) => sum + parseFloat(q.bobot || 0),
            0
        );
